...
export default class Map {
    ...
    switch (laneType) {
      case 'DOTTED_YELLOW':
        return drawDashedLineFromPoints(
          points, colorMapping.YELLOW, 1, 3, 3, this.zOffsetFactor, 1, false,
        );
      case 'DOTTED_WHITE':
        return drawDashedLineFromPoints(
          points, colorMapping.WHITE, 1, 0.5, 0.25, this.zOffsetFactor, 0.4, false,
        );
      case 'SOLID_YELLOW':
        return drawSegmentsFromPoints(
          points, colorMapping.YELLOW, 1, this.zOffsetFactor, false,
        );
      case 'SOLID_WHITE':
        return drawSegmentsFromPoints(
          points, colorMapping.WHITE, 1, this.zOffsetFactor, false,
        );
      case 'DOUBLE_YELLOW':
      ...
      case 'CURB':
        return drawSegmentsFromPoints(
          points, colorMapping.CORAL, 1, this.zOffsetFactor, false,
        );
      case 'HIGH_LIGHT':
        return drawSegmentsFromPoints(
          points, colorMapping.RED, 4, this.zOffsetFactor, false,
        );
      default:
        return drawSegmentsFromPoints(
  ...
  addLane(lane, coordinates, scene, type = '', isLeft = null) {
    const drewObjects = [];

    if (STORE.options.showMapCenterLane) {
    ...
    lane.rightBoundary.curve.segment.forEach((segment, index) => {
      const points = coordinates.applyOffsetToArray(segment.lineSegment.point);
      const boundary = this.addLaneMesh((type && (isLeft === false)) ? type : rightLaneType, points);
    ...
    lane.leftBoundary.curve.segment.forEach((segment, index) => {
      const points = coordinates.applyOffsetToArray(segment.lineSegment.point);
      const boundary = this.addLaneMesh((type && (isLeft === true)) ? type : leftLaneType, points);
  ...
  addBorder(borderPolygon, color, coordinates, scene) {
    ...
    const mesh = drawSegmentsFromPoints(
      border, color, 1, this.zOffsetFactor, true, false, 1.0,
    );
    scene.add(mesh);
  ...
  addCurve(lines, color, coordinates, scene) {
    ...
    lines.forEach((line) => {
      line.segment.forEach((segment) => {
        const points = coordinates.applyOffsetToArray(segment.lineSegment.point);
        const mesh = drawSegmentsFromPoints(
          points, color, 1, this.zOffsetFactor * 2, false,
        );
        scene.add(mesh);
        drewObjects.push(mesh);
      });
  ...
  shouldDrawObjectOfThisElementKind(kind) {
    ...
    return STORE.options.showHighMap && STORE.options[optionName] !== false;
  }
  ...
  updateIndex(hash, elementIds, scene) {
    if (STORE.hmi.inNavigationMode) {
      ...
      if ((hash !== this.hash || this.elementKindsDrawn !== newElementKindsDrawn) && MAP_WS.mapSocketIsConnect) {
        this.hash = hash;
  ...
  update(world, coordinates, scene) {
    this.removeDrewObjects(this.shouldersDrewObjects, scene);
    this.shouldersDrewObjects = [];
    if (STORE.options.showMapShoulder) {
      const shoulders = world.shoulders;
      if (!_.isEmpty(shoulders)) {
        const shouldersMap = {};
        shoulders.forEach(shoulder => {
          shouldersMap[shoulder.id.id] = shoulder.isLeft;
        });
        const shouldersArray = Object.keys(shouldersMap);
        // console.log('shouldersMap = ', shouldersMap);
        // console.log('shouldersArray = ', shouldersArray);
        if (this.data && this.data.lane) {
          this.data.lane.forEach(item => {
            const id = item.id.id;
            if (shouldersArray.includes(id)) {
              const drewObjects = this.addLane(item, coordinates, scene, 'HIGH_LIGHT', shouldersMap[id]);
              this.shouldersDrewObjects = this.shouldersDrewObjects.concat(drewObjects);
            }
          });
        }
      }
