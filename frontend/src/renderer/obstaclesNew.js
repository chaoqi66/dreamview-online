...
import { copyProperty, hideArrayObjects, calculateLaneMarkerPoints, convertToUpperCaseWithUnderscore, roundNumber } from 'utils/misc';
import {
  ...
  drawShapeFromPoints, drawHollowBox
} from 'utils/draw';
...
// 动目标类型
const DYNAMICS_OBSTACLES = [
  'TYPE_CAR',                 // 小型汽车
  'TYPE_BUS',                 // 公共汽车
  'TYPE_CONSTRN_VEH',         // 施工车辆
  'TYPE_TRICYCLE',            // 三轮车
  'TYPE_ARTICULATED_HEAD',    // 铰接式车辆头部
  'TYPE_ARTICULATED_BODY',    // 铰接式车辆车身
  'TYPE_CYC',                 // 自行车
  'TYPE_PED',                 // 行人
  'TYPE_TRUCK',               // 卡车
  'TYPE_BARROW',              // 手推车
...
const STATIC_OBSTACLES = [
  'TYPE_TRAFFIC_CONE',     // 交通锥
  'TYPE_WARN_TRIANGLE',    // 三角警告牌
  'TYPE_WATER_BARRIER',    // 水障
  'TYPE_LAMP_POST',        // 路灯杆
  'TYPE_TRAFFIC_SIGN',     // 交通标志
  'TYPE_WARN_POST',        // 警告标志
  'TYPE_TRAFFIC_BARREL',   // 交通桶
  'TYPE_VISION_OBSTACLE',  // 视觉障碍物
  'TYPE_STATIC_UNKNOWN',   // 静态未知物体
  'TYPE_CURB',             // 路缘
];

const BARRIER_TEXTS = {
  'TYPE_STRAIGHT_ARM_BARRIER': 'straight',
  'TYPE_FOLDING_ARM_BARRIER': 'folding',
  'TYPE_FENCE_ARM_BARRIER': 'fence',
};
...
export default class PerceptionObstaclesNew {
  ...
  update(world, coordinates, scene, camera, size, isBirdView) {
    ...
    if (camera !== undefined) {
      ...
      this.updateCarText(world, coordinates, scene, camera, size);
      this.updateLaneMarkers(world, coordinates, scene);
    }
  }

  updateCarText(world, coordinates, scene, camera, size) {
    if (!STORE.options.hideText) {
      const speed = world?.autoDrivingCar?.speed;
      const acceleration = world?.autoDrivingCar?.acceleration;
      let acc = '';
      if (acceleration >= 0) {
        acc = roundNumber(acceleration, 2);
      } else {
        acc = `— ${Math.abs(roundNumber(acceleration, 2))}`;
      }
      const position = coordinates.applyOffset(
        new THREE.Vector2(world.autoDrivingCar.positionX,
          world.autoDrivingCar.positionY)
      );
      const text = `${roundNumber(speed, 2)}/${acc}`;
      let color = 0x0de5de;
      ...
      this.addTextMesh(text, camera, color, {x: position.x, y: position.y, z: 5}, size, scene, 3);
    }
  }
  ...
  updateObjects(world, coordinates, scene, camera, size, isBirdView) {
    ...
    for (let i = 0; i < objects.length; i++) {
      ...
      if (STORE.options.showRangeView && obstacle.distance && obstacle.distance > STORE.meters.autoRange) {
        continue;
      }
      ...
      if (!STORE.options.showUniversalObstacles && !DYNAMICS_OBSTACLES.includes(obstacle.objectType)) {
        continue;
      }

      if (planningObstacles.some(item => item.isDeadCar === true)) {
      ...
      if (obstacle?.barrierGate?.state) {
        this.drawBarrier(obstacle, position, scene, camera);
        continue;
      }
      ...
      let decisionRetPair = STORE.meters.selectEfficientLaneSequence?.decisionRetPair || [];
      if (STORE.options.showRoadNet && STORE.meters.selectEgoPathProposal) {
        decisionRetPair = STORE.meters.selectEgoPathProposal?.decisionRetPair || [];
      }

      let predictionDecision = null;
      if (obstacle.isPrediction && obstacle.predictionDecision && obstacle.predictionDecision.length > 0) {
        predictionDecision = obstacle.predictionDecision[0];
      }
      if (decisionRetPair) {
      ...
      if (STORE.options.showObstaclesHeading) {
        if (DYNAMICS_OBSTACLES.includes(obstacle.objectType)) {
  ...
  drawBarrier(obstacle, position, scene, camera) {
    if (!STORE.options.showObstacleBarrier) {
      return;
    }
    const boxMesh = drawHollowBox({ length: obstacle.length, width: obstacle.width, height: obstacle.height}, 0xff0000);
    boxMesh.rotation.z = obstacle.heading;

    const angle = obstacle?.barrierGate?.angle || 0;
    boxMesh.updateMatrixWorld();
    const rotationMatrix = new THREE.Matrix4().makeRotationY(-angle);
    boxMesh.matrixWorld.multiply(rotationMatrix);
    boxMesh.rotation.setFromRotationMatrix(boxMesh.matrixWorld);

    const offsetX = Math.cos(obstacle.heading) * obstacle.length / 2;
    const offsetY = Math.sin(obstacle.heading) * obstacle.length / 2;
    const startPoint = new THREE.Vector2(position.x - offsetX, position.y - offsetY);
    boxMesh.position.set(startPoint.x, startPoint.y, 1);
    scene.add(boxMesh);
    this.bboxDoorList.push(boxMesh);

    const text = BARRIER_TEXTS[obstacle.objectType] || '';
    text && this.addTextMesh(text, camera, 0xff0000, {x: position.x, y: position.y, z: 1}, 0.6, scene);
  }
  ...
  updateTexts(adc, obstacle, obstaclePosition, scene, camera, size, isBirdView, isV2X) {
    if (STATIC_OBSTACLES.includes(obstacle.objectType) && !STORE.options.showPerceptionTypeStaticUnkonwn) {
      return;
    }
    ...
    if (STORE.options.showObstaclesId) {
      ...
      let existConfidenceText = EXIST_CCONFIDENCE_TEXT[obstacle.existConfidence] ? `/${EXIST_CCONFIDENCE_TEXT[obstacle.existConfidence]}` : '';
      let rwText = '';
      if (STORE.options.showRwText) {
        rwText = RW_TEXT[obstacle.intentionRoadRight] ? `/${RW_TEXT[obstacle.intentionRoadRight]}` : '';
      }
      if (!enterObstacleId || obstacle.id === enterObstacleId) {
        let text = `${obstacle.id}${existConfidenceText}${rwText}`;
        if (camera !== undefined && camera instanceof THREE.OrthographicCamera) {
          text = `${obstacle.id}${existConfidenceText}/${speed}${rwText}`;
        }
        if (obstacle.topic === '/bev_object') {
          existConfidenceText = obstacle.confidence ? `/${roundNumber(obstacle.confidence, 2)}` : '';
          text = `${obstacle.id}${existConfidenceText}${rwText}`;
          if (obstacle.innerTypeId) {
            text = `${obstacle.innerTypeId}${existConfidenceText}${rwText}`;
          }
        }
