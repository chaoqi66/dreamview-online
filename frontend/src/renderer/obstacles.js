...
const InteractLatColorMapping = {
  IGNORE_LAT: 0xDCDCDC,
  BYPASS_LEFT: 0x0000FF,
  BYPASS_RIGHT: 0x1E90FF,
};
const InteractLonColorMapping = {
  IGNORE_LON: 0xD3D3D3,
  FOLLOW: 0x00FF00,
  OVERTAKE: 0xFFFF00,
  YIELD: 0xFF0000
...
export default class PerceptionObstacles {
  ...
  update(world, coordinates, scene, camera, isBirdView) {
    this.resetObjects(scene, _.isEmpty(world.object));
    this.updateObjects(world, coordinates, scene, camera, isBirdView);
  ...
  updateObjects(world, coordinates, scene, camera, isBirdView) {
    ...
    for (let i = 0; i < objects.length; i++) {
      ...
      if (obstacle.predictionDecision && obstacle.predictionDecision.length > 0) {
        const predictionDecision = obstacle.predictionDecision[0];
        ...
        if (STORE.options.showPredictionDecision && interactLat instanceof Object) {
          ...
          if (maxKey) {
            color = InteractLatColorMapping[maxKey] || DEFAULT_COLOR;
          }
        }

        if (STORE.options.showPredictionDecision && interactLon instanceof Object) {
          const maxKey = this.findMaxKey(interactLon);
          if (maxKey) {
            const interactLonBottomColor = InteractLonColorMapping[maxKey] || DEFAULT_COLOR;
            const interactLonBottom = drawShapeFromPoints(
              coordinates.applyOffsetToArray(obstacle.polygonPoint),
              new THREE.MeshBasicMaterial({ color: interactLonBottomColor }), false, 2,
            );
      ...
      this.updateTexts(adc, obstacle, position, scene, camera, isBirdView, isV2X);
  ...
  updateTexts(adc, obstacle, obstaclePosition, scene, camera, isBirdView, isV2X) {
    const initPosition = {
    ...
    if (STORE.options.showObstaclesInfo) {
      ...
      this.drawTexts(`(${distance}, ${speed})`, initPosition, scene, camera);
      lineCount++;
    }
    if (STORE.options.showObstaclesId) {
      const textPosition = {
        x: initPosition.x + (lineCount * deltaX),
        y: initPosition.y + ((lineCount + 3.5) * deltaY),
        z: initPosition.z + (lineCount * deltaZ),
      };
      this.drawTexts(obstacle.id, textPosition, scene, camera);
      lineCount++;
    }
    if (STORE.options.showPredictionPriority) {
      ...
      if (priority && priority !== 'NORMAL') {
        ...
        this.drawTexts(priority, textPosition, scene, camera);
        lineCount++;
      }
    }
    if (STORE.options.showPredictionInteractiveTag) {
      ...
      if (interactiveTag && interactiveTag !== 'NONINTERACTION') {
        ...
        this.drawTexts(interactiveTag, textPosition, scene, camera);
        lineCount++;
      }
    }
    if (STORE.options.showObstacleStatus) {
      ...
      if (obstacleStatus) {
        ...
        this.drawTexts(obstacleStatus, textPosition, scene, camera);
        lineCount++;
      }
    }
    if (STORE.options.showGoalLaneInJunction) {
      ...
      if (goalLaneInJunction) {
        ...
        this.drawTexts(goalLaneInJunction, textPosition, scene, camera);
        lineCount++;
      }
    }
    if (isV2X) {
      _.get(obstacle,'v2xInfo.v2xType',[]).forEach((t) => {
        ...
        this.drawTexts(t, textPosition, scene, camera, 0xFF0000);
        lineCount++;
      });
    }
  }
  ...
  drawTexts(content, position, scene, camera, color = 0xFFEA00) {
    const text = this.textRender.drawText(content, scene, camera, color);
    if (text) {
