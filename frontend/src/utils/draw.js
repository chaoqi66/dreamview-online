...
export function drawImage(img, width, height, x = 0, y = 0, z = 0, zHeight = 1) {
  ...
  const mesh = new THREE.Mesh(new THREE.BoxGeometry(width, height, zHeight), material);
...
export function drawHollowEllipse(params) {
  const material = new THREE.LineBasicMaterial({
    color: 0xffffff,
    linewidth: 1,
  });
...
export function drawHollowBox(vehicleParam, color = 0xffffff) {
  const width = vehicleParam.width;
  const length = vehicleParam.length;
