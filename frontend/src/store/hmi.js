...
export default class HMI {
  ...
  @observable isMainConnected = false;
  ...
  @observable currentPointCloudColor = '0';

  @observable currentPointCloudSize = 0.05;
  ...
  @action updateStatus(newStatus) {
    ...
    if (newStatus.currentRecordStatus) {
      const { currTimeS, totalTimeS, playRecordStatus, currentRecordId } = newStatus.currentRecordStatus;
      this.currTimeS = currTimeS;
      this.playRecordStatus = playRecordStatus;
      this.currentRecordId = currentRecordId || '';
      this.totalTimeS = newStatus?.records[currentRecordId]?.totalTimeS;
    }
    if (newStatus.currentPointCloudColor) {
      this.currentPointCloudColor = newStatus.currentPointCloudColor;
    }
    if (newStatus.currentPointCloudSize) {
      this.currentPointCloudSize = newStatus.currentPointCloudSize;
    }
  }
  ...
  updateVehicleParam(vehicleParam) {
    ...
    this.vehicleParam.width = parseFloat(this.vehicleParam.width) + 0.08 + '';
  ...
  @action setIsMainConnected(flag) {
    this.isMainConnected = flag;
  }
