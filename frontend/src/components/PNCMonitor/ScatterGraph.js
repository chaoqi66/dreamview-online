...
export default class ScatterGraph extends React.Component {
  initializeCanvas(title, options) {
    ...
    const chartOptions = {
      title: {
        display: (title && title.length > 0) && !options.hideTitle,
        text: title,
      },
      legend: {
    ...
    if (options.axes) {
      ...
      for (const axis in options.axes) {
        ...
        const axisOptions = {
          id: `${axis}-axis-0`,
          type: setting.type || 'linear',
          scaleLabel: {
            display: !_.isEmpty(setting.labelString),
            labelString: setting.labelString,
          },
          ticks: {
            display: !setting.hidden,
            min: setting.min,
            max: setting.max,
            maxTicksLimit: setting.maxTicksLimit || 10,
            minRotation: 0,
            maxRotation: 0,
  ...
  updateChart(props) {
    ...
    if (datasets.polygons) {
      for (const name in datasets.polygons) {
        ...
        if (name.includes('_core') && properties.color) {
          properties.color = 'rgba(0, 255, 60, 0.5)';
        } else if (name.includes('_buffer') && properties.color) {
          properties.color = 'rgba(0, 255, 255, 0.5)';
        } else {
  ...
  render() {
    const {
      title, options, properties, data, isShow
    } = this.props;

    const style = {
      display: (isShow || isShow === undefined) ? 'block' : 'none',
    };
    return (
      <div className="scatter-graph" style={style}>
        <canvas ref={(input) => {
...
function generateScatterGraph(setting, lineDatasets, carDatasets, polygonsDatasets) {
  ...
  // console.log('setting 777 = ', setting);
  // console.log('lineDatasets 777 = ', lineDatasets);
  // console.log('carDatasets 777 = ', carDatasets);
  // console.log('polygonsDatasets 777 = ', polygonsDatasets);
