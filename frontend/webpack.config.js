...
let parametersFiles = glob.sync('src/store/config/parameters*.yml');
if (parametersFiles.length === 0) {
  parametersFiles.push('src/store/config/parameters.yml');
}
...
module.exports = (env, argv) => {
  ...
  return {
    ...
    entry: {
      app: './app.js',
      parameters: parametersFiles.map(file => path.join(__dirname, file))
    },
    ...
    plugins: [
      ...
      new webpack.DefinePlugin({
        ...
        CONFIG_FILES: JSON.stringify(parametersFiles)
      }),
